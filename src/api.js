  ...
  if (!accessToken || tokenCheck.error) {
    ...
    if (!code) {
      const results = await axios.get(
        'https://r1t3t9efoh.execute-api.us-east-2.amazonaws.com/dev/api/get-auth-url'
      );
...
export const getEvents = async () => {
  ...
  if (token) {
    removeQuery();
    const url = `https://r1t3t9efoh.execute-api.us-east-2.amazonaws.com/dev/api/get-events/${token}`;
...
const getToken = async code => {
  ...
  const { access_token } = await fetch(
    `https://r1t3t9efoh.execute-api.us-east-2.amazonaws.com/dev/api/token/${encodeCode}`
  )
    .then(res => res.json())
    .catch(error => error);
